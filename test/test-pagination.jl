...
@testset "paginate_cursor" begin
    ...
    items = collect(HeatpumpMonitorAPI.paginate_cursor(c -> pages[c]))
...
@testset "paginate_offset stops on short page" begin
    ...
    items = collect(HeatpumpMonitorAPI.paginate_offset(; page_size = 10) do offset, limit
...
@testset "paginate_pagenum stops on empty page" begin
    ...
    items = collect(HeatpumpMonitorAPI.paginate_pagenum(p -> pages[p]))
    @test items == ['a', 'b', 'c']
end
