...
@testset "TokenBucket immediate acquire" begin
    clk = _mock_clock()
    b = HeatpumpMonitorAPI.TokenBucket(; rate = 10.0, burst = 5.0)
    HeatpumpMonitorAPI.acquire!(b; sleep_fn = clk.sleep_fn, time_fn = clk.time_fn)
...
@testset "TokenBucket waits when empty" begin
    clk = _mock_clock()
    b = HeatpumpMonitorAPI.TokenBucket(; rate = 2.0, burst = 1.0)
    HeatpumpMonitorAPI.acquire!(b; sleep_fn = clk.sleep_fn, time_fn = clk.time_fn)
    HeatpumpMonitorAPI.acquire!(b; sleep_fn = clk.sleep_fn, time_fn = clk.time_fn)
...
@testset "TokenBucket throws when timeout would expire" begin
    clk = _mock_clock()
    b = HeatpumpMonitorAPI.TokenBucket(; rate = 1.0, burst = 1.0)
    HeatpumpMonitorAPI.acquire!(b; sleep_fn = clk.sleep_fn, time_fn = clk.time_fn)
    @test_throws HeatpumpMonitorAPI.RateLimitError HeatpumpMonitorAPI.acquire!(
...
@testset "with_rate_limit runs fn after acquire" begin
    clk = _mock_clock()
    b = HeatpumpMonitorAPI.TokenBucket(; rate = 100.0, burst = 5.0)
    n = Ref(0)
    HeatpumpMonitorAPI.with_rate_limit(b, () -> n[] += 1;
