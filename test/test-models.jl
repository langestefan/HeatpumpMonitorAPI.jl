...
@testset "Generated module surface" begin
    @test isdefined(HeatpumpMonitorAPI, :HeatpumpMonitorAPIAPI)
    api_mod = getfield(HeatpumpMonitorAPI, :HeatpumpMonitorAPIAPI)
    public = filter(n -> n !== :HeatpumpMonitorAPIAPI, names(api_mod; all = false))
    @test !isempty(public)
end
