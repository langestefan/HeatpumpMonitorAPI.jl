...
"""
    resolve_credentials(T::Type{<:Auth}; env_prefix="HEATPUMPMONITORAPI") -> T
...
Build an [`Auth`](@ref) of type `T` from the first available source:
  ...
  2. `~/.config/heatpumpmonitorapi/credentials.toml` keyed under the auth
...
const _DEFAULT_ENV_PREFIX = "HEATPUMPMONITORAPI"
const _CREDENTIALS_FILE =
    joinpath(get(ENV, "XDG_CONFIG_HOME", joinpath(homedir(), ".config")),
             "heatpumpmonitorapi", "credentials.toml")
